...
// Shared sections used by both the public Landing page and the signed-in
// MemberHome page, so the "green hero" and pillar cards are defined ONCE.
...
/*  Tokens                                                             */
/* ------------------------------------------------------------------ */
export const GREEN = "#19352d";
export const GREEN_DEEP = "#122721";
export const GREEN_MID = "#142e27";
export const AMBER = "#d99b26";
...
/* ------------------------------------------------------------------ */
/*  "Read more" expanded content (verbatim from the legacy site)       */
/* ------------------------------------------------------------------ */
const DETAILS = {
  Business: {
    heading: "Business Project Management Tools.",
    subheading: "We provide project management tools that help your business stay organized.",
    columns: [
      [
  ...
  Educate: {
    ...
    subheading: "Make it Automated if you want, so you can make the process much more efficient.",
    groups: [
      {
        label: "Getting Started",
        columns: [
          ["Create a New Group.", "Invite Members to the Group.", "Create a New Task then Make Assignments"],
          ["Automate the process.", "Make a Task for ALL Current Members.", "Or, Make a Task for ALL NEW Members."],
          ["Easy to Manage", "Review the Progress of Each Member.", "Allow them to learn at their own pace."],
        ],
      },
      {
        label: "Features",
        columns: [
          ["Lesson building software", "Quiz creation Software", "Quiz Tracking Software", "Individual or Group training"],
          ["Hands-off training for 24/7 results", "Educate people all over the world.", "From the comfort of your home."],
          ["Be Creative.", "You can reach a mass audience.", "Share what you know and help others."],
        ],
      },
    ],
...
/* ------------------------------------------------------------------ */
/*  1. Hero — used on BOTH the public Landing page and MemberHome.     */
/*     mode="public"  -> bottom strip shows Sign in / Get started      */
/*     mode="member"  -> bottom strip shows "Welcome {userName}!"      */
/* ------------------------------------------------------------------ */
export function Hero({ mode = "member", userName = "there" }) {
  return (
      ...
      <div className="border-t border-white/10 bg-black/15 px-8 sm:px-12 py-5 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        {mode === "member" ? (
          ...
          <div className="flex flex-wrap items-center gap-4">
            <Link to="/signin" className="text-base sm:text-lg font-semibold text-white/90 hover:text-white transition-colors">
              Sign in
            </Link>
            <Link
              to="/signup"
              className="rounded-full bg-[#d99b26] text-[#122721] font-bold text-[15px] sm:text-base px-5 py-2.5 hover:bg-[#e6ab3a] transition-colors"
            >
              Get started free
            </Link>
          </div>
...
/* ------------------------------------------------------------------ */
/*  2. Three pillar cards (with expandable "Read more" detail panel)   */
/* ------------------------------------------------------------------ */
export function PillarCards() {
  const [open, setOpen] = useState(null); // "Business" | "Educate" | "Personal" | null
  const toggle = (title) => setOpen((cur) => (cur === title ? null : title));

  return (
    <section className="mt-14 sm:mt-16">
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {PILLARS.map(({ title, Icon, items }) => {
          const isOpen = open === title;
          return (
            <article
              key={title}
              className={`relative rounded-2xl bg-white border shadow-[0_18px_40px_-28px_rgba(18,39,33,0.45)] pt-14 pb-8 px-7 mt-8 transition-all duration-300 ${
                isOpen ? "border-[#d99b26] ring-2 ring-[#d99b26]/30" : "border-gray-200/80"
              }`}
            >
              ...
              >
                ...
                <IconArrow className={`h-4 w-4 transition-transform duration-300 ${isOpen ? "rotate-90" : ""}`} />
              </button>
            </article>
          );
        })}
      ...
      {/* Expandable panel — grows BELOW the grid, never overlaps the cards */}
      <div
      ...
      >
        <div className="overflow-hidden">
          {open && (
            <div className="mt-8 rounded-2xl bg-white border border-[#d99b26]/40 shadow-[0_18px_40px_-28px_rgba(18,39,33,0.45)] p-8 sm:p-10">
              <div className="flex items-start justify-between gap-4">
                <div>
                  <h4 className="text-xl sm:text-2xl font-bold text-[#19352d]">{DETAILS[open].heading}</h4>
                  <p className="mt-2 whitespace-pre-line text-base text-gray-600">{DETAILS[open].subheading}</p>
                </div>
                <button
                  type="button"
                  onClick={() => setOpen(null)}
                  aria-label="Close"
                  className="rounded-full p-2 text-gray-400 hover:bg-gray-100 hover:text-[#19352d] transition-colors"
                >
                  ✕
                </button>
              </div>

              {(DETAILS[open].columns
                ? [{ label: null, columns: DETAILS[open].columns }]
                : DETAILS[open].groups
              ).map((group, gi) => (
                <div key={gi} className="mt-8">
                  {group.label && (
                    <p className="mb-4 text-center text-sm font-bold uppercase tracking-[0.12em] text-[#d99b26]">
                      {group.label}
                    </p>
                  )}
                  <div className="grid gap-8 md:grid-cols-3">
                    {group.columns.map((col, ci) => (
                      <ul key={ci} className="space-y-2.5">
                        {col.map((t, i) => (
                          <li key={i} className="flex items-start gap-2.5 text-[15px] leading-snug text-gray-700">
                            <span className="mt-2 h-1.5 w-1.5 shrink-0 rounded-full bg-[#d99b26]" />
                            <span>{t}</span>
                          </li>
                        ))}
              ...
              {DETAILS[open].quote && (
                <p className="mt-8 text-center text-lg font-semibold italic text-[#19352d]">
                  “{DETAILS[open].quote}”
                </p>
              )}
            </div>
...
export function InteractiveSoftware() {
  return (
