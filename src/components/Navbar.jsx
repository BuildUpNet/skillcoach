...
const more = (user) => [
  ["Community", [["Coaches corner", "/coaches-corner"], ["Members", "/members"], ["Forum", "/forums"], ["Summary", "/projects"]]],
  ["Account", [["My profile", `/profile/${user.username || user.user_id}`], ["Messages", "/messages"], ["Settings", "/settings"]]],
];

// Logged-out (public) nav — only these 3 links, per the approved public layout
const publicLinks = [
  { to: "/projects", label: "Projects" },
  { to: "/badges", label: "Badges" },
  { to: "/credits", label: "Credits" },
...
// Logged-out "More" dropdown — only these 2 items
const publicMore = [
      ...
      <div className="sticky top-0 z-40 px-4 pt-4">
        <header className="mx-auto flex w-full max-w-[1200px] items-center gap-4 rounded-2xl border border-white/60 bg-white/85 px-4 py-2.5 shadow-[0_10px_40px_-18px_rgba(20,26,24,.35)] backdrop-blur-xl lg:rounded-full lg:px-5">
          ...
          <nav className="mx-auto hidden items-center gap-1 lg:flex">
            ...
            <div ref={ref} className="relative">
              ...
              {menu && (
                <div className="absolute right-0 mt-3 w-52 rounded-2xl border border-line bg-white p-2 shadow-[0_24px_60px_-24px_rgba(20,26,24,.4)]">
                  {publicMore.map(([label, to]) => (
                    <Link key={to} to={to} onClick={() => setMenu(false)} className="block rounded-lg px-3 py-2 text-[15px] font-medium text-ink/80 hover:bg-forest-soft hover:text-forest">
                      {label}
                    </Link>
                  ))}
                </div>
          ...
          <div className="ml-auto hidden items-center gap-2 lg:flex">
            <Link to="/signin" className="rounded-full px-4 py-2 text-[15px] font-semibold text-ink/70 hover:bg-forest-soft hover:text-forest transition-colors">
              Sign in
            </Link>
            <Link to="/signup" className="rounded-full bg-gold px-5 py-2.5 text-[15px] font-bold text-ink shadow-[0_8px_20px_-10px_rgba(217,164,65,.9)] transition-transform hover:-translate-y-px hover:bg-gold-deep hover:text-white">
              Sign up
            </Link>
          </div>
        ...
        {open && (
          <div className="mx-auto mt-2 w-full max-w-[1200px] rounded-2xl border border-line bg-white p-3 shadow-xl lg:hidden">
            {publicLinks.map((l) => (
              <NavLink key={l.to} to={l.to} onClick={() => setOpen(false)} className={({ isActive }) => `block rounded-xl px-4 py-3 text-[16px] font-semibold ${isActive ? "bg-forest text-white" : "text-ink/80"}`}>
                {l.label}
              </NavLink>
            ))}
            <div className="mt-2 border-t border-line pt-2">
              <p className="px-4 pb-1 text-xs font-bold uppercase tracking-wider text-gold-deep">More</p>
              {publicMore.map(([label, to]) => (
                <Link key={to} to={to} onClick={() => setOpen(false)} className="block rounded-xl px-4 py-2.5 text-[15px] font-medium text-ink/80">
                  {label}
                </Link>
              ))}
            </div>
            <Link to="/signin" onClick={() => setOpen(false)} className="mt-3 block rounded-xl border border-line px-4 py-3 text-center text-[16px] font-semibold text-ink">
              Sign in
            </Link>
            <Link to="/signup" onClick={() => setOpen(false)} className="mt-2 block rounded-xl bg-gold px-4 py-3 text-center text-[16px] font-bold text-ink">
              Sign up
            </Link>
          </div>
        )}
